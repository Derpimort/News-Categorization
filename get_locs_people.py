...
def ent_destroyer(row):
    ...
    try:
        if pd.isna(sentence):
            ...
            tags, toptag, people=get_locs(tokenized.ents)
        ...
        toptag=''
        people=['']
    return row.name, tags, toptag, people
...
if __name__=="__main__":
    ...
    coords_proc.join()
    result.columns=['id', 'locs', 'top_loc', 'people']
    result=result.set_index('id')
    result.to_csv(DATA_DIR+"entities_full.csv")
    result.to_pickle(DATA_DIR+"entities_full.pkl")
    print("Done!")
